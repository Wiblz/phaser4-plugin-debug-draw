...
const scene: Phaser.Types.Scenes.SettingsConfig & {
  ...
  create: function (this: Phaser.Scene) {
    ...
    const mesh = this.add.mesh(512, 192, 'elephant');
    ...
    // Create points array for rope (10 points evenly spaced vertically)
    const ropePoints: Phaser.Types.Math.Vector2Like[] = [];
    for (let i = 0; i < 10; i++) {
      ropePoints.push({ x: 0, y: i * 10 });
    }

    const rope = this.add.rope(768, 192, 'elephant', undefined, ropePoints);

    rope.setName('elephantRope');
    rope.setDebug(this.debugDraw.graphic);

    this.add.tween({
      targets: rope.points,
      // @ts-expect-error Phaser type bug: config parameter should be optional (see StaggerBuilder signature)
